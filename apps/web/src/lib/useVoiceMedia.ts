...
export function useVoiceMedia({ socket, user, iceServers, voiceRoomIds, microphoneDeviceId = "", microphoneVolume = 100, noiseSuppression = DEFAULT_NOISE_SUPPRESSION, afkRoomIds = [] }: UseVoiceMediaInput) {
  ...
  const ensurePeer = useCallback((peerUserId: string) => {
    ...
    peer.onconnectionstatechange = () => {
      ...
      const state: PeerConnectionState = peer.connectionState === "connected" && !recovering
        ? "connected"
  ...
  const ensureInitialOffer = useCallback((peerUserId: string, peer: RTCPeerConnection | null) => {
    const currentUserId = userIdRef.current;
    if (!peer || !currentUserId || offeredPeersRef.current.has(peer)
      || makingOfferPeersRef.current.has(peerUserId)
      || !shouldInitiatePeerConnection(currentUserId, peerUserId)) return;
    void sendOffer(peerUserId, peer).catch(() => setError("voiceError.startPeer"));
  }, [sendOffer]);

  const recoverPeer = useCallback((peerUserId: string, expectedPeer?: RTCPeerConnection) => {
    const peer = peersRef.current.get(peerUserId);
    ...
    const restartTimeout = window.setTimeout(() => {
      ...
      if (state.reason === "transport" && peer.connectionState === "connected") {
        peerRecoveryStatesRef.current.set(peerUserId, advancePeerRecovery(state, { type: "connected" }, Date.now()).state);
        setPeerConnectionStates((current) => ({ ...current, [peerUserId]: "connected" }));
        return;
      }
      schedulePeerRecovery(peerUserId, peer, { type: "restart_failed" });
  ...
  const handleSignal = useCallback(async (payload: { fromUserId: string; mediaInstanceId?: string; signal: RtcSignal }) => {

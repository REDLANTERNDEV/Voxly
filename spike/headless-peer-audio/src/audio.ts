...
/** The Track this spike plays. Synthesised, so nothing here is licensed audio. */
...
/**
 * Read an Ogg Opus file into the packets that go on the wire.
 *
 * Hand-rolled rather than taken from a library because it is the shape the real
 * bot needs anyway: ffmpeg will hand it encoded Opus and it will packetise that
 * itself. werift's own file player only accepts MP4 and WebM.
 */
...
/** How many frames playback owes by `elapsedMs`. Drives the send pacing. */
...
/**
 * The `index`th frame of playback, looping the Track so the person testing has
 * as long as they need to put headphones on.
 */
 ...
 * holding another Listener's header, so each Listener gets its own.
 */
